...
#define ll long long
#define pb push_back
int n,q,p[100005],si[100005],t[600000],dep[100005],fa[100005],he[100005],ord[100005],ordt=0,ordi[100005],top[100005];
...
inline int query(int l,int r,int x,int lx,int rx){
    ...
    return (query(l,r,2*x,lx,m))^(query(l,r,2*x+1,m+1,rx));
}
    ...
    for(int i=1;i<=q;i++){
        ...
        if(c==1) update(ord[u],v,1,1,n);
            ...
            while(top[u]!=top[v]){
                ...
                ans=ans^query(ord[top[u]],ord[u],1,1,n);
                u=fa[top[u]];
            }
            if(dep[u]>dep[v]) swap(u,v);
            ans=ans^query(ord[u],ord[v],1,1,n);
            cout<<ans<<endl;
        }
    }
    return 0;
