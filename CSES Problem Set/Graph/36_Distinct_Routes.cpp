...
public:
    ...
    long long dfs_paths(int u,long long curFlow){
        pathT++;
        path[pathT]=u;
        if((curFlow==0)||(u==n)){
            cout<<pathT<<endl;
            for(int i=1;i<=pathT;i++){
                cout<<path[i]<<" ";
            }cout<<endl;
            pathT--;
            return curFlow;
        ...
        for(;ver[u].cur<ver[u].edges.size();ver[u].cur++){
            ...
            int v=ver[u].edges[ver[u].cur];
            //cout<<v<<" "<<flow[u][v]<<endl;
            if(flow[u][v]<=0){
                continue;
            }
            long long tryy=dfs_paths(v,min(curFlow-res,flow[u][v]));
            res+=tryy;
            flow[u][v]-=tryy;
            if(res==curFlow){
                pathT--;
